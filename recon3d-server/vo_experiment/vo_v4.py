...
while True:
    ...
    if len(shifts) >= 4:
        ...
        # 섹터 시프트 합의도: 부호가 갈리면(발산/수렴 혼재=중심 부정확 잔여) 신뢰↓
        sg = np.sign(shifts); agree = abs(sg.sum()) / len(sg)
        conf *= (0.4 + 0.6 * agree)
        if center_hold:           # 중심 불안정 프레임: 배제 대신 신뢰만 하향
            conf *= 0.3
...
np.savetxt(f"{OUT}/vo_v4_{TAG}.csv", arr, delimiter=",",
